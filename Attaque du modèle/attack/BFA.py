...
class BFA(object):
    ...
    def flip_bit(self, m):
        ...
        w_grad_topk, w_idx_topk = m.weight.grad.detach().abs().reshape(-1).topk(k_top)
        # update the b_grad to its signed representation
        #w_grad_topk = m.weight.grad.detach().view(-1)[w_idx_topk]
        w_grad_topk = m.weight.grad.detach().reshape(-1)[w_idx_topk]
        ...
        w_bin = int2bin(m.weight.detach().reshape(-1), m.N_bits).short()
    ...
    def progressive_bit_search(self, model, data, target):
        ...
        while self.loss_max <= self.loss.item():
            ...
            for name, module in model.named_modules():
                if isinstance(module, quan_Conv2d) or isinstance(module, quan_Linear) or isinstance(module, quan_Conv1d):
